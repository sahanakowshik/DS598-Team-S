...
def main():
    ...
    model = MultipleAgentsNet(policy_net, num_agents=len(SUBMISSION_ID_LIST)).to(device)
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if torch.cuda.device_count() > 1:
            torch.save(model.module.policy_net.state_dict(), model_name)
        else:
            torch.save(model.policy_net.state_dict(), model_name)
